...
export const PATCH = createApiHandler(async (request, context) => {
  ...
  if (id === undefined) throw new ApiError(422, apiErrorCodes.validationError, "El tipo de clase no es válido.");
  return Response.json(await (await getAuthService()).updateAdminSchedulingCatalog(request, context.correlationId, "class-types", id, await readValidatedJson(request, validateUpdateSchedulingCatalog)));
});
