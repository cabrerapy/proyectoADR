...
export const GET = createApiHandler(async (request) => {
  ...
  return Response.json(await (await getAuthService()).listAdminSchedulingCatalog(request, "class-types", result.data));
});
export const POST = createApiHandler(async (request, context) => Response.json(
  await (await getAuthService()).createAdminSchedulingCatalog(request, context.correlationId, "class-types", await readValidatedJson(request, validateCreateSchedulingCatalog)),
  { status: 201 },
));
