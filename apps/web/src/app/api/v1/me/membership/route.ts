import { validateOwnMembershipQuery, type ValidationIssue } from "@gym-adr/validation";
...
export const GET = createApiHandler(async (request) => {
  const result = validateOwnMembershipQuery(new URL(request.url).searchParams);
  if (!result.success) {
    throw new ApiError(422, apiErrorCodes.validationError, "La consulta no es válida.", {
      fieldErrors: fieldErrors(result.issues),
    });
  }
  return Response.json(await (await getAuthService()).getOwnMemberships(request, result.data));
});
