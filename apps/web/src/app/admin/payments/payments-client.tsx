...
interface PaymentView { readonly amount: number; readonly id: string; readonly paidAt: string; readonly status: string; readonly userId: string; readonly version: number }
interface PaymentQueryPage { readonly capabilities?: { readonly canCorrect: boolean }; readonly cursor?: string; readonly payments: readonly PaymentView[] }
...
function PaymentCorrectionForm({ onCompleted, payment }: { readonly onCompleted: () => void; readonly payment: PaymentView }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault(); if (busy) return;
    const form = event.currentTarget; const data = new FormData(form); const type = String(data.get("type"));
    setBusy(true); setError(""); setMessage(""); requestKey.current ??= crypto.randomUUID();
    const idempotencyKey = requestKey.current;
    try {
      const response = await fetch(`/api/v1/admin/payments/${encodeURIComponent(payment.id)}/corrections`, {
        body: JSON.stringify({
          ...(type === "VOID" ? {} : { amount: Number(data.get("amount")) }),
          expectedVersion: payment.version, originalPaidAt: payment.paidAt, originalPaymentId: payment.id,
          reason: String(data.get("reason")), type, userId: payment.userId,
        }),
        headers: { "content-type": "application/json", "idempotency-key": idempotencyKey }, method: "POST",
      });
      if (!response.ok) throw await failure(response, "No pudimos corregir el pago.");
      setMessage("Corrección registrada correctamente."); requestKey.current = undefined; form.reset(); onCompleted();
    } catch (reason: unknown) { setError(reason instanceof Error ? reason.message : "No pudimos corregir el pago."); }
    finally { setBusy(false); }
  };
  return <details className="mt-3 border-t border-black/10 pt-3"><summary className="cursor-pointer font-bold">Corregir pago</summary><form className="mt-3 grid gap-3" onChange={() => { requestKey.current = undefined; }} onSubmit={(event) => void submit(event)}><label className="font-bold">Tipo<select className="mt-1 min-h-11 w-full rounded-lg border border-slate-400 px-3" name="type"><option value="VOID">Anulación</option><option value="ADJUSTMENT">Ajuste</option><option value="COMPENSATION">Compensación</option></select></label><label className="font-bold">Importe del ajuste o compensación<input className="mt-1 min-h-11 w-full rounded-lg border border-slate-400 px-3" min={1} name="amount" step={1} type="number" /></label><label className="font-bold">Motivo<textarea className="mt-1 min-h-20 w-full rounded-lg border border-slate-400 p-3" minLength={5} maxLength={500} name="reason" required /></label><button className="min-h-11 rounded-lg bg-brand-900 px-4 font-bold text-white disabled:opacity-60" disabled={busy} type="submit">{busy ? "Guardando…" : "Confirmar corrección"}</button><p aria-live="assertive" className="font-semibold text-red-700">{error}</p><p aria-live="polite" className="font-semibold text-emerald-700">{message}</p></form></details>;
}
