...
| TASK-009 | INCOMPLETA | 2026-08-04 | Compose aislado y scripts de up/readiness/smoke/down implementados con AWS SDK v3, tabla efímera On-Demand, `TransactWriteItems`, lecturas fuertes y limpieza ante error. Cinco pruebas del tooling, formato, lint, typecheck, build y las 24 pruebas CDK ejecutadas en serie pasan; la web local responde 200 y muestra su estructura inicial. `npm run dynamodb:local:test` continúa fallando de forma controlada porque Docker no está instalado; falta el smoke real para completar la tarea. No hubo conexión ni despliegue AWS. |
