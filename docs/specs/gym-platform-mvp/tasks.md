...
| TASK-008 | INCOMPLETA | 2026-08-04 | Next.js 16.2.12 y OpenNext 4.0.2 compatibles y fijados; CDK sintetiza con fixture S3 privado SSE-S3, CloudFront OAC y Lambdas server/image con URL IAM, roles mínimos y cero DynamoDB. `npm ci`, check, 24 pruebas CDK, builds y synth local/development con fixture pasan. Pendiente: `open-next build` compila Next.js pero el binario esbuild no puede resolver archivos bajo el sandbox Windows; WSL/Docker no están instalados. No hubo deploy. |
